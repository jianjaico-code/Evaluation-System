<?php
	...
	if(isset($_POST['active'])) {
		...
		echo "hello";
		$data->hide_comment($id, $conn);
	}elseif(isset($_POST['deactive'])){
		$id = $_POST['activate_stats'];
		$data->unhide_comment($id, $conn);
...
<?php echo $facID -> SY_Name?><br>
	...
	<?php if($process == 1):?>
	<form method="POST">
		<button type="submit" class="float-right btn btn-primary btn-lg" name="proceed">Proceed</button>
	</form>
	<?php endif; ?>
		<button class="float-right btn btn-primary btn-lg" onclick="exportData()">Export to Excel</button>
		<a style="margin-right: 10px;" href="view_report_edited.php?Subject_Offer_ID=<?php echo $id;?>" title="Print Edited Report" class="btn btn-primary btn-lg float-right">Next</a>
		<a style="margin-right: 10px;" href="index.php" title="Back" class="btn btn-primary btn-lg float-right"id="noPrint">Back</a>
	</div>
</div>
		...
		<h1 class="text-center">General Comments</h1><br>
			<div class="table-responsive">
				<table class="table table-bordered stick-top">
					...
					<?php while($res2 = $result2 -> fetch_object()): ?>
					<tbody class="body_data">
						<td width="90%"><?php echo $res2 -> Comments?></td>
						<td class="text-center">
							<?php if($res2 -> Status =='Active') echo '<a href="#deactive_account" data-toggle="modal" data-id='.$res2->Student_Evaluation_ID.' class="activate" style="color:green;">Hide</a>'; ?>
							<?php if($res2 -> Status =='Inactive') echo '<a href="#active_modal" data-toggle="modal" data-id='.$res2 -> Student_Evaluation_ID.' class="activate" style="color:red;">Unhide</a>'; ?>
						</td>
					</tbody>
					<?php endwhile;?>	
